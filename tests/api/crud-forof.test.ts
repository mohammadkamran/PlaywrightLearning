...
test.describe("Booking CRUD API Suite (Data-driven using for...of)", () => {
  let token: string;
  ...
  for (const [index, createPayload] of createData.entries()) {
    const updatePayload = updateData[index];

    test(`CRUD flow for booking index ${index} -> ${createPayload.firstname}`, async ({ request }) => {
      ...
      // ---------- 4) READ AFTER UPDATE ----------
      const getAfterUpdate = await request.get(`${BASE_URL}/booking/${bookingId}`);
      ...
      const getAfterDelete = await request.get(`${BASE_URL}/booking/${bookingId}`);
      expect(getAfterDelete.status()).toBe(404);

      console.log(`GET after DELETE [${index}] status =`, getAfterDelete.status());
    });
  }
});
